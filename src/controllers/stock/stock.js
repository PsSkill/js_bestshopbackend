...
exports.post_stocks = async (req, res) => {
    ...
    if (valueSets.length === 0) {
      return res.status(400).json({ err: "No valid size and quantity combinations provided" });
    }
  
    const placeholders = valueSets.map(() => "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").join(", ");
    ...
    try {
      const flattenedValues = valueSets.flatMap(values => values);
      await post_query_database(query, flattenedValues);
